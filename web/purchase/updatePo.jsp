...
<script language="javascript" type="text/javascript">
	...
	var payType = $("input[name=payType2]").attr("value");
	$("select[name=payType] option[value="+payType+"]").attr("selected","selected");
	if(payType=="预付款到发货"){
	$("input[name=prePayFee]").removeAttr("readonly");
	}
	no=document.getElementById("poitem").rows.length-1;
	$("input[name=max]").attr("value",no);
	var x=1;
	for(;x<=no;x++){
		for(var g=1;g<=document.getElementById("spxxTable").rows.length;g++){
			if($("input[name=productCode"+x+"]").attr("value")==$("#spxxTable tr:eq("+g+") td:eq(1)").text()){
				$("input[name=productName"+x+"]").attr("value",(document.getElementById('spxxTable').rows[g].cells[2].innerHTML));
				}
			}
		}
	}
...
<form action="../purchaseservlet/PomainUpdateSaveServlet" method="post" id="pomain">
<table width="100%"  border="0" align="center" cellspacing="1">
  <tr>
    <td class="title1">采购单编号</td>
    <td class="title1"><input type="text" readonly="readonly" name="poId" value="${pomain.poId }" id="pomaintable"></td>
	<td class="title1">供应商</td>
	<c:set var="venderName" value="123"></c:set>
	<c:forEach items="${vList }" var="vender"><c:if test="${pomain.venderCode==vender.venderCode }"><c:set var="venderName" value="${vender.name }"></c:set></c:if></c:forEach>
	...
	<td class="title1"><input name="venderCode" readonly="readonly" value="${pomain.venderCode }-------${venderName }"><img onClick='changediv2(this)' style='cursor:hand' src='../images/selectDate.gif'></td>
    <td class="title1">创建用户</td>
    <td class="title1"><input type="text" name="account" value="${ScmUser.account }"></td>
    <td class="title1">付款类型</td>
    <td class="title1"><select name="payType" onchange="yufukuan()"><option value="货到付款">货到付款</option><option value="款到发货">款到发货</option><option value="预付款到发货">预付款到发货</option></select></td>
<!--     <td class="title1">操作</td> -->
  </tr>
  <tr>
      <td class="title1">附加费用</td>
    <td class="title1"><input type="text" name="tipFee" value="${pomain.tipFee }" onchange="setpot()"></td>
        <td class="title1">采购产品总价</td>
    <td class="title1"><input type="text" name="productTotal" value="${pomain.productTotal }" readonly="readonly"></td>
        <td class="title1">采购单总价</td>
    <td class="title1"><input type="text" name="poTotal" value="${pomain.poTotal }" readonly="readonly"></td>
        <td class="title1">预付款</td>
    <td class="title1"><input type="text" name="prePayFee" value="${pomain.prePayFee }" readonly="readonly" onchange="zhengze()"></td>
  </tr>
  <tr>
      <td class="title1">采购单状态</td>
    <td class="title1"><input type="text" name="status" value="${pomain.status }" readonly="readonly"></td>
    <td class="title1">创建时间</td>
    <td class="title1"><input type="text" name="createTime" value="${pomain.createTime }" readonly="readonly"></td>
    <td class="title1" colspan="4" >备注<input size="100" type="text" name="remark" value="${pomain.remark }"></td>
  </tr>
  <tr>
  	<td class="title2"></td>
  </tr>
...
<table width="100%"  border="0" cellpadding="0" cellspacing="0" id="poitem">
	...
	<c:forEach items="${poitem }" var="poi">
	<c:set var="no2" value="${no2+1 }"></c:set>
	<tr name="${no2 }">
	<td align=center>${no2 }</td>
	<td align=center><input readonly="readonly" type="text" name="productCode${no2 }" value="${poi.productCode }"><img onClick="changediv(this)" style="cursor:hand" src="../images/selectDate.gif"></td>
	<td align=center><input readonly="readonly" type="text" name="productName${no2 }"></td>
	<td align=center><input readonly="readonly" type="text" name="unitName${no2 }" value="${poi.unitName }"></td>
	<td align=center><input readonly="readonly" type="text" name="unitPrice${no2 }" value="${poi.unitPrice }"></td>
	<td align=center><input onchange="sum(this)" type="text" name="num${no2 }" value="${poi.num }"></td>
	<td align=center><input readonly="readonly" type="text"  name="itemPrice${no2 }" value="${poi.itemPrice }"></td>
	<td align=center><span onClick="deleted(this)" style="cursor:hand">×</span></td>
	</tr>
	</c:forEach>
</table>
...
<input type="hidden" name=payType2 value="${pomain.payType }">
</div>
